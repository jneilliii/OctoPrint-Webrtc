/*
 * View model for OctoPrint-Webrtc
 *
 * Author: jneilliii
 * License: AGPLv3
 */
$(function() {
    ...
    function WebrtcViewModel(parameters) {
        var self = this;
        ...
        self.settings.testWebcamStreamUrl = function () {
            if (!self.settings.webcam_streamUrl()) {
                return;
            }

            if (self.settings.testWebcamStreamUrlBusy()) {
                return;
            }
            ...
            var webcam_element;
            if (streamType == "mjpg") {
                webcam_element = $('<img src="' + self.settings.webcam_streamUrl() + '">');
                ...
                if (video_element.canPlayType("application/vnd.apple.mpegurl")) {
                    video_element.src = self.settings.webcam_streamUrl();
                ...
                startWebRTC(video_element, self.settings.webcam_streamUrl());
            } else {
            ...
            self.settings.testWebcamStreamUrlBusy(true);
            showMessageDialog({
                title: gettext("Stream test"),
                message: message,
                onclose: function () {
                    self.settings.testWebcamStreamUrlBusy(false);
                }
            });
        };
        ...
        self.control._disableWebcam = function () {
        ...
        self.control._enableWebcam = function () {
            if (
                OctoPrint.coreui.selectedTab != "#tab_plugin_webrtc" ||
        ...
        self.onTabChange = function (current, previous) {
            if (current == "#tab_plugin_webrtc") {
                self.control._enableWebcam();
            } else if (previous == "#tab_plugin_webrtc") {
